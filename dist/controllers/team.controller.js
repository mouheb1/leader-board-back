...
import { createTeamSchema, updateTeamSchema, updateScoreSchema, } from '../validators/team.validator.js';
...
export async function joinTeam(req, res) {
    const user = await teamService.addMemberToTeam(req.params.id, req.user.userId);
    res.json({
        status: 'success',
        data: { user },
    });
...
export async function updateTeamScoreAsMember(req, res) {
